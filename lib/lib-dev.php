...
define('CRITERIA_COURSE_GRADE', 250003); // added by G. Schwed, 2016-01
...
function block_dukreminder_filter_users($entry) {
    ...
    if ($entry->criteria > 1000000) {
        ...
        $entry->criteria = CRITERIA_COURSE_GRADE; // = 25000
    ...
    mtrace("<br>processing reminder $entry->id: $type / subject: $entry->title / criteria: $criteria_name / $details<br/>"); // Schwed debug
    ...
    if ($entry->dateabsolute > 0 && $entry->criteria == CRITERIA_COMPLETION) {
        ...
        foreach ($users as $user) {
            ...
            if (($timecompleted)) {
                ...
                mtrace("... user $user->id: course completed $timecompleted => skipped<br/>"); // Schwed debug
                unset($users[$user->id]);
            }
        }
        return $users;
    ...
    if ($entry->dateabsolute > 0 && $entry->criteria == CRITERIA_COURSE_GRADE) {
        ...
        foreach ($users as $user) {
            ...
            if ($grade AND $grade->finalgrade >= $grade_item->gradepass) {
                #mtrace("... user $user->id grade: $grade->rawgrade; finalgrade: $grade->finalgrade; => passed => skipped<br>"); // schwed debug
                $detail .= " => passed => skipped";
                unset($users[$user->id]);
            } else { $detail .= " => not passed!";
            }
            mtrace("... user $user->id: $detail<br>"); // schwed debug
        }
        return $users;
    }
    ...
    if ($entry->daterelative > 0 && $entry->criteria == CRITERIA_COURSE_GRADE) {
        ...
        // Get all grades for specified item.
        #$grades = $DB->get_records('grade_grades', array('itemid' => $grade_item->id), '', 'userid, id, itemid, rawgrade, finalgrade, timecreated, timemodified');
        #print_r($grades);
        #print "<br>-----------<br>"; // Schwed debug
        ...
        if ($grade_item->itemmodule == 'quiz') {
            ...
            #print_r($quiz);
            // Calculate grading factor (max. grade / total of question points).
            $g = $quiz->grade / $quiz->sumgrades; //
            #print "<br>Faktor: $g<br>";
            ...
            foreach ($users as $user) {
                ...
                if ($quiz_attempt) {
                    ...
                    mtrace("no succesful attempt => skipped<br>");
                    unset($users[$user->id]);
                    continue;
                }
                // Check time
        ...
        if ($grade_item->itemmodule == 'scorm') {
            ...
            $course = $DB->get_record('course', array('id' => $entry->courseid));
            #print "<br>Hier kommt ein SCORM:<br>";
            #print_r($scorm);

            // Find the last passed attempt and check grade and time
            foreach ($users as $user) {
                mtrace("... user $user->id: ");
                // Get last passed scorm attempt for user.
                #$result = scorm_user_outline($course, $user, '', $scorm); // ok
                #print "Scorm-user-outline: $result->info; ". userdate($result->time)."<br>"; 
                #$result = scorm_user_complete($course, $user, '', $scorm); // zu viel
                #$result = scorm_get_user_grades($scorm, $user->id); //
                #$result = scorm_grade_user($scorm, $user->id);

                // Get all attempts for this user.
                $result = scorm_get_attempt_count($user->id, $scorm, '', '');
                print "$result attempt(s): "; 

                $attempts = scorm_get_all_attempts($scorm->id, $user->id);
                foreach ($attempts as $i) {
                    #$times = array();
                    $grade[$i] = scorm_grade_user_attempt($scorm, $user->id, $i);
                    if ($grade[$i] >= 15) { // todo gradepass !!!
                        $times[$i] = scorm_get_sco_runtime($scorm->id, '', $user->id, $i)->finish;
                        print " attempt $i: $times[$i] ";
                    } else {
                        print " not yet passed! => skipped<br>"; 
                        unset($users[$user->id]);
                        unset($times);
                    }
                }
                ...
                if (isset($times)) {
                    ...
                    print "=> latest: $la; grade = $grade[$i]; finished ".userdate($times[$la])."<br>";
                    unset($times);
                }
    ...
    if ($entry->daterelative > 0 && $entry->criteria != CRITERIA_COMPLETION && $entry->criteria != CRITERIA_ENROLMENT) {
        ...
        foreach ($users as $user) {
            $usercompleted = $completion->get_user_completion($user->id, $criteria);
            # $usercompleted->timecompleted; // unnötig
...
function block_dukreminder_get_criteria($criteriaid) {
    ...
    if ($criteriaid > 1000000) { // 
        ...
        #print "grade_item_id: $grade_item_id"; // Schwed debug
        #$criteriaid = CRITERIA_COURSE_GRADE; // eigentlich unwichtig
        ...
        #return get_string('criteria_all', 'block_dukreminder'); // Schwed to do
        return 'letzte positive Bewertung von: ' .  $gradeitem->itemname . " (" . $gradeitem->itemmodule . ")"; // Schwed todo
    }
...
/**
 * Get quiz details and last passed attempt of users
 * @param object $quiz
 * @param array $users - list of potential users
 * @return array $users - users to recieve a reminder
 */
function block_dukreminder_get_quiz_results($quiz) {
    global $DB;
